...
- [x] ~~`@wasmagent/trust-cli` npm publish~~ — publishing from this incubation repo violates project boundaries; AgentBOM CLI release belongs in standalone `WasmAgent/agentbom` after schema stabilization, and MCP Posture CLI release belongs in `wasmagent-js` after runtime scanning primitives stabilize. Revert PR #285 publish workflow.
