...
/**
 * Canonical AgentBOM schema source (issue #355 / #357).
 *
 ...
 * (schema id `agentbom`, version `agentbom/v0.1`, registered in the package's
 * `schemas/index.json`). We prefer the canonical package copy and fall back to the
 * locally vendored `specs/agentbom/schema.json` only when the package is absent or has
 * not registered the schema. The two copies are content-identical today; a repo-level
 * drift gate tracking their continued equality lands with the rest of #355.
 */
...
function getValidator(): ValidateFunction {
  ...
  const schema = loadAgentBomSchema() as { $schema?: string } & Record<string, unknown>;
  // The canonical schema from @wasmagent/protocol declares draft 2020-12 while the vendored
  // specs/agentbom/schema.json declares draft-07; their bodies are otherwise identical. ajv's
  // default constructor validates schema documents against the draft-07 meta-schema and rejects
  // a `$schema` it has no meta-schema for (it throws
  // `no schema with key or ref "https://json-schema.org/draft/2020-12/schema"`). Dropping the
  // declaration makes ajv treat both copies as draft-07 — the body's actual dialect — so
  // validation behaviour is identical regardless of which copy loads.
